...
export class CounterComponent {
  ...
  public startCounter(element: HTMLDivElement): void {
    ...
    const [diffDigits, diffDecimals] = String(this.to - this.from)
      ...
      .split('.')
      .map(str => str.split('').map((_, index, array) => index === 0 ? '0' : array.slice(0, index).join('')));

    const sequences = [];

    diffDigits.forEach((numString, index) => {
      const klass = this.sequenceClasses[index];
      const num = +numString;
      const anims = [];
      ...
      sequences.push(query(klass, animate(`${this.animationSeconds}s ease-in-out`, keyframes([ ...anims ])), { optional: true }));
    });

    const diffDecimalsBase = diffDigits.join('');

    diffDecimals.forEach((numString, index) => {
      const klass = this.sequenceClasses[diffDigits.length - 1 + index];
  ...
  private buildSequenceNumbers(): IFromToSequenceNumbers {
    ...
    return {
      from: {
        digits: this.digits.slice(0, this.digits.length - fromDigits.length).concat([...fromDigits].map(Number)),
        decimals: this.decimals.slice(0, this.decimals.length - fromDecimals.length).concat([...fromDecimals].map(Number))
      },
      to: {
        digits: this.digits.slice(0, this.digits.length - toDigits.length).concat([...toDigits].map(Number)),
        decimals: this.decimals.slice(0, this.decimals.length - toDecimals.length).concat([...toDecimals].map(Number))
      }
    }
  }
  ...
  private buildSequenceClasses(): ISequenceClasses {
    return {
      digits: this.sequenceNumbers.to.digits.map((_, index, { length }) => `digit-${new Array(length - index).fill(0).join('')}`),
      decimals: this.sequenceNumbers.to.decimals.map((_, index) => `decimal-${new Array(index + 1).fill(0)}`)
    }
  }
}
